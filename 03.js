...
function pelisEstrenos(listaDePeliculas) {
  //Tu código aquí:
  let nacho = listaDePeliculas;

  return function (arr){
    if(arr.length === 0)return "Por favor indicar las peliculas que son de estreno";
for(var i = 0; i < nacho.length; i++){
  for(var x of arr){
    if(nacho[i].nombre === x)nacho[i].estreno = true;
}
}
for(var i = 0; i < nacho.length;i++){
if(nacho[i].estreno === true){
  return nacho;
}
}
